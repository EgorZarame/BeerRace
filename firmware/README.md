...
1. **Start Game** — режим ожидания кружки.
2. Поставить **пустую** кружку → **Cup Placed** — весы обнуляются (tare), на экране ≈0 г.
3. **Налить** напиток — на экране виден вес жидкости (например 450 г).
4. **DRINK!** — система ждёт, **не** запускает таймер.
5. **Поднимите** кружку с весов (вес падает ≥50 г) → **старт таймера**.
6. Пьёте, кружка **не** на весах — таймер идёт, финиш **не** срабатывает.
7. **Финиш** — пустая кружка на весах, средний вес **от 3 до 30 г** от нуля (±шум датчика), ~0.4 с стабильно.

> Не убирайте кружку с весов до финиша — иначе вес упадёт до 0 и игра может завершиться раньше.
